(ns stay.operation
  "OperationActor — one register/place/report/dispute operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (StayBooking-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the StayGovernor (:govern) and the
  ...
  decision to a human reviewer (guest-relations officer). The reviewer
...
(defn build
  "Compiles an OperationActor graph bound to `store` (any `stay.store/Store`).
  opts:
    :advisor      — a `stay.llm/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; StayBooking-LLM inference (the contained intelligence node) — proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (llm/-advise advisor store request)]
            {:proposal p :audit [(llm/trace request p)]})))

      ;; StayGovernor — independent censor (separate system than the LLM).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        ...
                        :reason (or reason
                                    (cond (:dispute? verdict) :booking-dispute
                                          (:flagged? verdict) :flagged-guest
                                          :else :low-confidence))
                        :phase ph
      ...
      ;; (guest-relations officer) resumes with :approval. Then route
      ;; commit/hold.
